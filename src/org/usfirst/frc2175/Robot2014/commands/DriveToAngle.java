...
public class  DriveToAngle extends Command {
    ...
    public DriveToAngle(float angle, boolean isRelative) {
	...
	//Robot.drivetrain.resetGyro();
        double desiredHeading = inputAngle;
        if (isRelative)
            desiredHeading += Robot.drivetrain.getGyroAngle();
	Robot.drivetrain.gyroPID.reset();
	Robot.drivetrain.gyroPID.setSetpoint(desiredHeading);
...
//	Robot.drivetrain.gyroPID.setPID(SmartDashboard.getNumber("P"),SmartDashboard.getNumber("I")/1000,0);
     ...
     * 
     * This is probably a bad termination condition... can we make sure that
     * this terminates properly when the bot is only momentarily at the right
     * heading?  i.e. momentum carries it through.
    */
